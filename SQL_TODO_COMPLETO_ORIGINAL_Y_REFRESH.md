# ✅ SQL COMPLETO - Migración Original + Refresh Cache

## 📋 Copia y Pega TODO Esto en Supabase SQL Editor

Este SQL incluye:
1. ✅ La migración completa original (crea tablas, columnas, foreign keys)
2. ✅ Verificación y creación de foreign keys si faltan
3. ✅ Refresh del cache de PostgREST

---

```sql
-- ============================================================================
-- RanchLink v1.0 Production Schema Sync Migration (COMPLETA)
-- ============================================================================
-- 
...
--
-- INCLUDES: Full schema creation + PostgREST cache refresh
--
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- 9. REFRESCAR POSTGREST SCHEMA CACHE (CRÍTICO)
-- ----------------------------------------------------------------------------
-- PostgREST cachea el schema para performance. Después de crear foreign keys,
-- necesita refrescar su cache para reconocer las relaciones.
-- 
...
-- ----------------------------------------------------------------------------
-- 10. Verificación Final - Mostrar todas las foreign keys de tags
-- ----------------------------------------------------------------------------
SELECT 
  tc.constraint_name,
  tc.table_name,
...
-- PostgREST cache has been refreshed.
--
-- NEXT STEPS:
-- 1. Wait 10-30 seconds for PostgREST to reload
-- 2. Go to: https://ranch-link.vercel.app/superadmin
-- 3. Try "Generate & Mint Tags" - should work without errors
-- ============================================================================
```

---

## ✅ Qué Hace Este SQL Completo

1. ✅ **Crea/actualiza `animals`** - Agrega `id` (uuid PRIMARY KEY) y todas las columnas v1.0
2. ✅ **Crea `ranches`** - Tabla de ranchos
3. ✅ **Crea `tags`** - Tabla principal de tags con todas las columnas
4. ✅ **Actualiza `batches`** - Agrega `batch_name` y todas las columnas v1.0
5. ✅ **Crea `kits` y `kit_tags`** - Tablas para kits de retail
6. ✅ **Crea todas las foreign keys** - Incluyendo `tags_animal_id_fkey` (el más importante)
7. ✅ **Habilita RLS** - Row Level Security en todas las tablas
8. ✅ **Crea índices** - Para performance
9. ✅ **Refresca PostgREST cache** - Con `NOTIFY pgrst, 'reload schema'`
10. ✅ **Muestra resumen** - De todas las foreign keys creadas
...
- ✅ Tabla al final mostrando todas las foreign keys de `tags`
- ✅ PostgREST cache refrescado
- ✅ El error "Could not find a relationship" desaparece

---

## 📝 Instrucciones

1. **Copia TODO el SQL de arriba** (desde `-- ============================================================================` hasta el final)
2. **Pega en Supabase SQL Editor**
3. **Click "Run"**
4. **Espera 10-30 segundos** para que PostgREST recargue
5. **Prueba:** https://ranch-link.vercel.app/superadmin → "Generate & Mint Tags"

---

**Este SQL incluye TODO: la migración original completa + el refresh del cache.** ✅
