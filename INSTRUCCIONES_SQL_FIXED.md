...
**Causa:** La tabla `animals` existe pero usa `public_id` como primary key, no `id` (uuid).

---

## ✅ SOLUCIÓN

He creado un SQL corregido que:
1. **Agrega la columna `id` (uuid) a `animals`** si no existe
2. **Hace `id` el primary key** (mantiene `public_id` como único)
3. **Crea todas las foreign keys** después de asegurar que las columnas existen
...
### 2. Copia este SQL CORREGIDO completo:

El archivo está en: `supabase/migrations/PROD_SYNC_V1_FIXED.sql`

**O copia directamente desde aquí** (todo el bloque de abajo):

---

## 📄 SQL COMPLETO CORREGIDO
...
2. **Copia TODO el SQL de arriba** (desde `-- ============================================================================` hasta el final)
...
1. **Agrega `id` (uuid) a `animals`** - Convierte `public_id` de primary key a unique, y hace `id` el primary key
2. **Crea tabla `tags`** - Si no existe
3. **Agrega `batch_name` a `batches`** - La columna faltante
4. **Crea todas las foreign keys** - Después de verificar que las columnas existen
5. **Crea índices** - Para rendimiento

---

**Este SQL está corregido y debería funcionar sin errores.** ✅
