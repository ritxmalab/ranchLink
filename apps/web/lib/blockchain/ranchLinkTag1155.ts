...
function getClients() {
  ...
  const contractAddress = (process.env.RANCHLINKTAG_1155_ADDRESS || '0x559D8f4196578c2308e092225bdFbBc1AF88d0Fb') as `0x${string}`
...
export async function lazyMintTag(
  ...
  // Parse TagActivated event to get tokenId
  const tagActivatedTopic = '0x' + Buffer.from(
    'TagActivated(uint256,bytes32,address,string)'
  ).toString('hex')
